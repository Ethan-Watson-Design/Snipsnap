...
/// Frontmost-app metadata plus an on-screen window snapshot, captured before Snipsnap UI takes focus.
...
final class CaptureDestinationMappingCache {
    ...
    private init() {
        ...
        storageDirectory = appSupport.appendingPathComponent("Snipsnap/auto-organize", isDirectory: true)
...
enum CaptureClassifier {
    ...
    /// Frontmost app/window plus on-screen geometry snapshot — call before Snipsnap UI takes focus.
    ...
    static func printSaveDiagnostics(image: NSImage, windowInfo: WindowSignature) async {
        print("\n[Snipsnap AutoOrganize] ── Classification inputs ──")
    ...
    private static func snapshotOnScreenWindows() -> WindowGeometrySnapshot {
        let snipsnapBundleID = Bundle.main.bundleIdentifier
        let snipsnapPID = ProcessInfo.processInfo.processIdentifier
    ...
    /// surfacing as part of the folder name (e.g. "GitHub - snipsnap").
